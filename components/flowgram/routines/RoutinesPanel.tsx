...
import { Routine, RoutineFrequency, ROUTINE_FREQUENCIES } from "@/lib/flowgram/types";
import {
  Repeat,
  Plus,
  Trash2,
...
export default function RoutinesPanel({ forDate }: RoutinesPanelProps) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/flowgram/routines");
      if (res.ok) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (editingId) {
      await fetch("/api/flowgram/routines", {
        method: "PATCH",
  ...
  async function stopFromHere(routine: Routine) {
    setPending(routine.id);
    try {
      await fetch(`/api/flowgram/routines?id=${routine.id}&from_date=${forDate}`, {
        method: "DELETE",
      });
  ...
  async function deleteFully(routine: Routine) {
    setPending(routine.id);
    try {
      await fetch(`/api/flowgram/routines?id=${routine.id}`, { method: "DELETE" });
      setConfirmingDelete(null);
      await load();
    } finally {
      setPending(null);
