...
export default function EventItemsManager({ eventId }: EventItemsManagerProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [linkedRes, itemsRes] = await Promise.all([
        fetch(`/api/flowgram/event-items?event_id=${eventId}`),
  ...
  async function addItem() {
    ...
    try {
      ...
      if (!item) {
        const itemRes = await fetch("/api/flowgram/items", {
          method: "POST",
      ...
      await fetch("/api/flowgram/event-items", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          event_id: eventId,
  ...
  async function removeLink(id: string) {
    setPending(true);
    try {
      await fetch(`/api/flowgram/event-items?id=${id}`, { method: "DELETE" });
      await load();
    } finally {
      setPending(false);
    }
