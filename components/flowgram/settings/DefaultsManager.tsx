...
export default function DefaultsManager() {
  ...
  async function load() {
    ...
    try {
      const [itemsRes, defRes, catRes] = await Promise.all([
        ...
        fetch("/api/flowgram/custom-categories"),
      ]);
  ...
  async function addItemToCategory(category: string) {
    ...
    try {
      ...
      if (!item) {
        const res = await fetch("/api/flowgram/items", {
          method: "POST",
      ...
      await fetch("/api/flowgram/category-defaults", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function removeDefault(defaultId: string) {
    setPending(true);
    try {
      await fetch(`/api/flowgram/category-defaults?id=${defaultId}`, {
        method: "DELETE",
      });
      await load();
    } finally {
  ...
  async function addCustomCategory() {
    ...
    setPending(true);
    try {
      await fetch("/api/flowgram/custom-categories", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function deleteCustomCategory(id: string, label: string) {
    ...
    setPending(true);
    try {
      await fetch(`/api/flowgram/custom-categories?id=${id}`, { method: "DELETE" });
      await load();
    } finally {
      setPending(false);
    }
