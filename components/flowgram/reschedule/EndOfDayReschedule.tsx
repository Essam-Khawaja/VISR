...
import { TimelineEvent } from "@/lib/flowgram/types";
import {
  formatTime,
  isPastEvent,
  getDurationMinutes,
...
export default function EndOfDayReschedule({
  ...
  async function fetchSlots(date: string, event: TimelineEvent) {
    ...
    try {
      const res = await fetch(
        `/api/flowgram/free-time?date=${date}&min_minutes=${minutes}`
      );
      if (res.ok) {
  ...
  return (
    <>
      ...
      {mounted && scheduling && createPortal(
        <div
          className="fixed inset-0 z-[100] bg-stone-900/40 backdrop-blur-sm flex items-center justify-center p-3 sm:p-6"
          onClick={closePicker}
        >
          <div
