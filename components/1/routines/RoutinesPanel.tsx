...
import { Routine, RoutineFrequency, ROUTINE_FREQUENCIES } from "@/lib/1/types";
import {
  Repeat,
  Plus,
  Trash2,
...
import {
  ...
  RoutineStatus,
} from "@/lib/1/routine-schedule";
import { isSameDay } from "@/lib/1/timeline-utils";
...
export default function RoutinesPanel({ forDate }: RoutinesPanelProps) {
  ...
  const [confirmingDelete, setConfirmingDelete] = useState<Routine | null>(
    null
  );
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/1/routines");
      if (res.ok) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (editingId) {
      await fetch("/api/1/routines", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: editingId, ...body, ends_on: null }),
      });
    } else {
      await fetch("/api/1/routines", {
        method: "POST",
  ...
  async function markIncomplete(routine: Routine) {
    setPending(routine.id);
    try {
      await fetch("/api/1/routines", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ id: routine.id, mark_incomplete: true }),
      });
      await load();
    } finally {
      setPending(null);
  ...
  async function stopFromHere(routine: Routine) {
    setPending(routine.id);
    try {
      await fetch(`/api/1/routines?id=${routine.id}&from_date=${forDate}`, {
        method: "DELETE",
      });
  ...
  async function deleteFully(routine: Routine) {
    setPending(routine.id);
    try {
      await fetch(`/api/1/routines?id=${routine.id}`, { method: "DELETE" });
      setConfirmingDelete(null);
      await load();
    } finally {
      setPending(null);
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              {frequency === "every_n_days" ? (
                ...
                <TimePicker value={preferredTime} onChange={setPreferredTime} />
              )}
            </div>
            <button
              type="submit"
            ...
            <div className="px-5 py-4 space-y-2 overflow-y-auto">
              ...
              >
                <p className="text-xs font-semibold text-amber-800">
                  Stop from this day forward
                </p>
                <p className="text-[11px] text-amber-700/80 mt-0.5 leading-relaxed">
                  Keeps history before {forDate}. The routine won&apos;t recur
                  from this day on.
                </p>
              </button>
              <button
              ...
              >
                ...
                <p className="text-[11px] text-rose-700/80 mt-0.5 leading-relaxed">
                  Removes the routine and all of its history.
                </p>
              </button>
            </div>
        ...
        className={`w-6 h-6 rounded-md border-2 shrink-0 flex items-center justify-center transition-colors ${
          isDone
            ...
            : isDue
              ...
              : "border-stone-300"
        } ${!isToday ? "cursor-not-allowed opacity-60" : "cursor-pointer"}`}
