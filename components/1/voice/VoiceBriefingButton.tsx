...
import { isRoutineScheduledOnDate } from "@/lib/1/routine-schedule";
import { blocksForDate } from "@/lib/1/personal-time";
...
export default function VoiceBriefingButton({
  ...
  return (
    <button
      onClick={speak}
      className="w-full glass-card px-4 py-3 flex items-center gap-3 hover:bg-white/85 transition-colors"
    >
      <div
        className={`w-9 h-9 rounded-full bg-gradient-to-br ${speaking ? "from-amaranth to-thulian" : "from-brook to-sage"} flex items-center justify-center shadow-soft`}
      >
        {speaking ? (
