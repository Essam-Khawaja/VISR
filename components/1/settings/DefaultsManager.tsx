...
export default function DefaultsManager() {
  ...
  async function addItemToCategory(category: string) {
    ...
    try {
      ...
      if (!item) {
        const res = await fetch("/api/1/items", {
          method: "POST",
      ...
      await fetch("/api/1/category-defaults", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function removeDefault(defaultId: string) {
    setPending(true);
    try {
      await fetch(`/api/1/category-defaults?id=${defaultId}`, {
        method: "DELETE",
      });
      await load();
    } finally {
  ...
  async function addCustomCategory() {
    ...
    setPending(true);
    try {
      await fetch("/api/1/custom-categories", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function deleteCustomCategory(id: string, label: string) {
    ...
    setPending(true);
    try {
      await fetch(`/api/1/custom-categories?id=${id}`, { method: "DELETE" });
      await load();
    } finally {
      setPending(false);
    }
