...
import { isoDateFromDate, todayISODate, isSameDay } from "./timeline-utils";
...
export function routinesForDate(routines: Routine[], date: Date): Routine[] {
  return routines.filter((r) => isRoutineScheduledOnDate(r, date));
}
...
export function dateIsoFromValue(value: Date | string): string {
  return typeof value === "string" ? value : isoDateFromDate(value);
}
