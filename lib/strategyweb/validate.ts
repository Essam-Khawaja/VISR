...
// The university onboarding form sends additional fields (endOfUniversityGoal,
// recurringCommitments, taskSeeds, etc.) and may post a short bottleneck
// concern as `brainDump`. We accept those and let extra fields pass through so
// the API can use them (e.g. `endOfUniversityGoal` overrides `destination`).
export const ProfileSchema = z
  .object({
    university: z.string().max(120).default(""),
    degree: z.string().max(120).default(""),
    year: z.string().max(40).default(""),
    targetGoal: z.string().min(1).max(280).default(""),
...
// Onboarding-time seed payload sent to /api/strategyweb/generate. The client
// generates these locally with a placeholder planId and the server re-IDs them
// to the real planId before persisting.
export const GenerateRequestWithSeedsSchema = z.object({
  profile: ProfileSchema,
  seedNodes: z.array(StrategyNodeSchema).optional(),
  seedTasks: z.array(StrategyTaskSchema).optional(),
});
...
export const OnboardingInsightRequestSchema = z.object({
  step: z.enum([
    ...
    "semester",
    "tasks",
    // Legacy aliases retained so older clients can still post insights:
    "courses",
    "commitments",
    "constraints",
    "brain-dump",
